...
const DISPLAY_NAMES = Object.freeze({ orcarouter: 'OrcaRouter', 'google-flow': 'Google Flow', 'google-veo': 'Google Veo', 'google-imagen': 'Google Imagen', 'google-gemini': 'Google Gemini', 'openai-images': 'OpenAI Images', vidu: 'Vidu', omni: 'Omni' });
const publicSetting = (row, defaults = []) => ({ provider: row.provider, name: DISPLAY_NAMES[row.provider], capabilities: CAPABILITIES[row.provider] || [], baseUrl: row.base_url, organizationId: row.organization_id, region: row.region, defaultModel: row.default_model, timeout: row.timeout_ms, retry: row.retry_count, enabled: Boolean(row.enabled), hasApiKey: Boolean(row.api_key_encrypted), apiKey: row.api_key_encrypted ? '••••••••' : '', defaultCapabilities: defaults, isDefault: defaults.length > 0, updatedAt: row.updated_at });
const CAPABILITIES = Object.freeze({ orcarouter: ['text'], 'google-flow': ['image', 'video'], 'google-veo': ['video'], 'google-imagen': ['image'], 'google-gemini': ['image'], 'openai-images': ['image'], vidu: ['image', 'video'], omni: ['image', 'video'] });
