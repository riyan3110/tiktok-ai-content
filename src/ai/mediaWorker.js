...
class MediaGenerationWorker {
  constructor({ db, transport, concurrency = 2 } = {}) { this.db = db; this.transport = transport; this.concurrency = concurrency; this.pending = []; this.running = new Set(); this.listeners = new Map(); }
  ...
  async run(job) { try { await connector.execute(this.db, job, this.transport, (status, id) => this.emit(id, status), job.id); } catch (error) { this.emit(job.id, 'Failed', { message: error.message }); } }
