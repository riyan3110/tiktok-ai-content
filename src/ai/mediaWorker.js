...
class MediaGenerationWorker {
  ...
  async run(job) { try { const result = await connector.execute(this.db, job, this.transport, (status, id) => this.emit(id, status), job.id); if (result.status === 'Completed') await this.onCompleted?.(job.id); } catch (error) { this.emit(job.id, 'Failed', { message: error.message }); } }
