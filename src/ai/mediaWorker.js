...
class MediaGenerationWorker {
  ...
  enqueue(body) { if (body.mediaType !== 'video') connector.validateGeneration(this.db, body); const id = body.id || crypto.randomUUID(); this.pending.push({ ...body, id }); queueMicrotask(() => this.drain()); return id; }
  ...
  async run(job) { try { const result = await connector.execute(this.db, job, this.transport, (status, id) => this.emit(id, status), job.id); if (result.status === 'Completed') await this.onCompleted?.(job.id); } catch (error) { const current = connector.generation(this.db, job.id); const storageStage = ['Downloading media', 'Uploading to COS'].includes(current?.status); this.db.prepare("UPDATE ai_generations SET status='Failed',error_type=COALESCE(error_type,?),error_code=COALESCE(error_code,?),error_message=COALESCE(error_message,?),updated_at=CURRENT_TIMESTAMP WHERE id=? AND status<>'Cancelled'").run(storageStage ? 'Storage Error' : (error.type || error.name || 'Worker Error'), error.code || (storageStage ? 'STORAGE_FAILURE' : null), error.message || 'Generation gagal', job.id); this.emit(job.id, 'Failed', { message: error.message }); } }
