...
class ProviderAdapter {
  ...
  buildRequest({ prompt, model, stream = false, mediaType = 'text', assets = [], parameters = {}, metadata = {} }) { return { model: model || this.config.default_model, prompt, type: mediaType, assets, ...parameters, metadata, stream }; }
  ...
  parse(data) { const content = data.output ?? data.text ?? data.content ?? data.result ?? data.choices?.[0]?.message?.content ?? '';
    const usage = data.usage || {}; const media = data.media || data.images || data.videos || (data.url ? [{ url: data.url }] : []); return { content: typeof content === 'string' ? content : JSON.stringify(content), media: Array.isArray(media) ? media : [media], providerJobId: data.id || data.job_id || null, status: data.status, usage: { promptTokens: usage.prompt_tokens || usage.input_tokens || 0, completionTokens: usage.completion_tokens || usage.output_tokens || 0, totalTokens: usage.total_tokens || 0 }, raw: data }; }
...
class GenericMediaAdapter extends ProviderAdapter {
  async execute(input, options = {}) {
    const first = await super.execute(input, options);
    if (!first.providerJobId || !['queued', 'pending', 'processing', 'running'].includes(String(first.status || '').toLowerCase())) return first;
    const { signal, onProgress = () => {} } = options; const interval = Math.max(100, Number(this.config.poll_interval_ms) || 2000); const maxPolls = Math.max(1, Number(this.config.max_polls) || 150);
    for (let poll = 0; poll < maxPolls; poll += 1) {
      if (signal?.aborted) throw Object.assign(new Error('Request cancelled'), { name: 'AbortError' });
      onProgress(poll ? 'Generating' : 'Waiting'); await new Promise((resolve, reject) => { const timer = setTimeout(resolve, interval); signal?.addEventListener('abort', () => { clearTimeout(timer); reject(Object.assign(new Error('Request cancelled'), { name: 'AbortError' })); }, { once: true }); });
      const response = await this.transport(this.endpoint(`${this.requestPath()}/${encodeURIComponent(first.providerJobId)}`), { headers: this.headers(), signal });
      if (!response.ok) throw Object.assign(new Error(await response.text() || `HTTP ${response.status}`), { status: response.status });
      const result = this.parse(await response.json()); const status = String(result.status || '').toLowerCase();
      if (['failed', 'error', 'cancelled'].includes(status)) throw new Error(result.content || `Provider job ${status}`);
      if (['completed', 'succeeded', 'success', 'done'].includes(status) || (!status && (result.content || result.media.length))) { onProgress('Rendering'); return result; }
    }
    throw Object.assign(new Error('Provider polling timed out'), { status: 504 });
  }
}
