...
class StorageService {
  ...
  async delete(id, { permanent = false } = {}) { const asset = this.repository.get(id); if (!asset) return false; if (!permanent) return Boolean(this.repository.trash(id)); await this.adapter(asset.storage_provider).delete(asset.storage_key); return Boolean(this.repository.remove(id)); }
  async deleteMany(ids, { permanent = true } = {}) { if (!Array.isArray(ids) || ids.length > 100) throw Object.assign(new Error('assetIds must be an array with at most 100 items'), { status: 422 }); const unique = [...new Set(ids.map(String))]; const deleted = []; const failed = []; for (const id of unique) { try { (await this.delete(id, { permanent })) ? deleted.push(id) : failed.push({ id, error: 'Asset tidak ditemukan' }); } catch (error) { failed.push({ id, error: error.message }); } } return { deleted, failed }; }
}
