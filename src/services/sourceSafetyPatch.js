...
const HARD_SOURCE_BOILERPLATE = /(?:editorial\s+(?:independence|policy|standards)|independent\s+editorial|ethics\s+policy|our\s+editorial\s+standards|maintains?\s+(?:its\s+)?editorial\s+independence|work\s+has\s+appeared\s+in|works?\s+has\s+appeared\s+in|has\s+(?:also\s+)?written\s+for|previously\s+(?:worked|wrote|reported)\s+(?:at|for)|before\s+joining|more\s+by\s+[A-Z]|follow\s+(?:the\s+)?(?:author|reporter|writer)|contact\s+(?:the\s+)?(?:author|reporter|writer)|author\s+(?:bio|profile)|writer\s+(?:bio|profile)|contributor\s+(?:bio|profile)|about\s+the\s+author|tentang\s+penulis|independensi\s+editorial|kebijakan\s+editorial|standar\s+editorial|karya(?:nya)?\s+(?:pernah\s+)?(?:muncul|dimuat)\s+di|pernah\s+menulis\s+untuk)/iu;
