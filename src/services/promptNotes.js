...
function list(db, query = '') {
  ...
  const rows = db.prepare('SELECT * FROM (SELECT rowid AS sort_rowid, * FROM prompt_notes ORDER BY datetime(created_at) DESC, rowid DESC LIMIT 500) ORDER BY datetime(created_at) ASC, sort_rowid ASC').all();
