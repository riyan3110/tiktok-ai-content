...
async function generateAndSave({ db, mode = 'ai', requestedTopic, category = 'Iklan & UGC', customCategory, format = 'Tutorial langkah', content = defaultContent, images = defaultImages, trending = defaultTrending, mainTopic = null, angle = null, useTrendReference = true, forceNewAngle = false }) {
  ...
  for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      const slides = await images.createSlides(renderKey, { ...generated, contentCategory, contentFormat });
