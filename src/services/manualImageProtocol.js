...
async function generate({ baseUrl, apiKey, model, format, prompt, images = [], signal }, transport) {
  protocol(format);
  ...
  if (format === 'router-images') {
    ...
    body = { model, prompt, n: 1, provider: { allow_fallbacks: false }, input_references: images.map(url => ({ type: 'image_url', image_url: { url } })) };
  } else if (format === 'chat') {
    endpoint = `${base}/chat/completions`;
    body = { model, messages: [{ role: 'user', content: [{ type: 'text', text: prompt }, ...images.map(url => ({ type: 'image_url', image_url: { url } }))] }], modalities: ['image', 'text'], stream: false };
...
module.exports = { protocol, generate };
