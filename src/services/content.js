...
async function generateContent(previousTopics, options = {}, client) {
  ...
  const trendDirection = options.trendReference ? `Referensi tren aktif memiliki tiga daftar terpisah. KEYWORD/HASHTAG: ${options.trendReference.keywords.join(' | ') || 'tidak ada'}; gunakan hanya untuk memilih istilah dan konteks yang relevan. GAYA HOOK: ${(options.trendReference.trend_hooks || []).join(' | ') || 'tidak ada'}; gunakan hanya sebagai referensi kalimat pembuka, jangan menyalin hook mentah terus-menerus dan buat variasi yang natural. POLA KONTEN: ${(options.trendReference.trend_content_patterns || []).join(' | ') || 'tidak ada'}; gunakan hanya sebagai referensi struktur penyampaian. Jangan mencampurkan ketiganya sebagai satu daftar. Dari setiap daftar, pilih hanya referensi yang cocok dengan topik dan kategori; jangan memaksakan referensi. Pilih nol sampai maksimal 3 keyword dan isi trendKeywordsUsed dengan keyword terpilih persis. Jangan ubah inti topik atau menyatakan viral, trending nomor satu, atau sedang ramai kecuali catatan ini menyatakannya: "${options.trendReference.notes || ''}".` : 'Tidak ada referensi tren aktif; isi trendKeywordsUsed dengan array kosong.';
