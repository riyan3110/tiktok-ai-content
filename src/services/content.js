...
function validateContent(content, { format = 'Tutorial langkah' } = {}) {
  ...
  if (words(content.hook).length > 16) errors.push(`Slide 1 memiliki ${words(content.hook).length} kata, batas maksimal 16 kata.`);
...
module.exports = { generateContent, generateAngles, validateContent, validateSlides, normalizeSlides, numberedValues, mainSlideText, slideWordLimit, MAX_REPAIR_ATTEMPTS };
