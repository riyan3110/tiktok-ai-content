...
const { resolveContentLayout, layoutInstruction, contentFormatForLayout, validateLayoutSlides } = require('./contentLayouts');

const LAYOUT_LABELS = { tutorial: 'Tutorial', story: 'Cerita', news: 'Berita' };
...
async function generateContent(previousTopics, options = {}, client) {
  ...
  const writingFormat = contentFormatForLayout(contentLayout, format);
  ...
  const formatStructures = {
    'Masalah dan solusi': 'Gunakan 4–5 slide. Mulai dengan satu slide ber-section MASALAH yang spesifik, lanjutkan 2–3 slide SOLUSI yang masing-masing membahas tindakan berbeda, lalu akhiri dengan HASIL atau PENUTUP yang relevan. Gunakan bullet tanpa nomor dan maksimal 3 point per slide.',
    'Tutorial langkah': 'Gunakan 4 slide secara default: PEMBUKA, dua slide LANGKAH yang membagi urutan tindakan secara seimbang, lalu HASIL/PENUTUP. Gunakan 5 slide jika langkah atau konteks memang membutuhkan ruang tambahan. Nomor langkah harus berurutan dan jangan membuat slide hanya untuk satu kalimat pendek.',
    'Fakta singkat': 'Gunakan 4–5 slide dengan alur fakta atau penjelasan yang natural. Dilarang memakai section atau copy TUTORIAL, LANGKAH, LANGKAH PRAKTIS, maupun penomoran langkah.',
    Listicle: 'Gunakan 4–5 slide Listicle. Slide pembuka memuat topik utama dan ringkasan singkat yang bermakna; setiap slide isi memuat satu item atau fakta berbeda; slide terakhir memuat item terakhir atau kesimpulan substantif.',
  ...
  const formatStructure = options.useSources || baselineSpecificFormats.has(writingFormat)
    ? formatStructures[writingFormat] || genericStructure
    : genericStructure;
  const sourceStructure = options.useSources
    ? `SOURCE menentukan fakta dan ${contentLayout === 'default' ? 'FORMAT' : 'TATA LETAK'} menentukan cara penyajian. Pertahankan struktur ${contentLayout === 'default' ? 'format' : 'tata letak'} di atas. Jika FACT_BANK memiliki beberapa fakta relevan yang berbeda, gunakan sebanyak mungkin tanpa mengulang: prioritaskan fakta utama, angka/statistik penting, nama entitas/pemain, metode/proses, perbandingan, lalu konteks penting. Jangan memakai slide title-only, CTA generik, credit sumber, atau ajakan membaca sumber selama masih ada fakta relevan yang belum dipakai; setiap slide wajib memiliki body atau points yang bermakna. Title, body, dan points harus saling melengkapi dengan informasi berbeda. Jika hanya ada 1–2 fakta, sederhanakan penyajian tanpa membuat filler seolah fakta dan jangan mengarang. Untuk Tutorial, tindakan hanya boleh ditulis bila didukung sumber. Untuk Cerita, jangan membuat kejadian, tokoh, emosi, atau sebab-akibat yang tidak ada di sumber. Untuk Berita, fakta terpenting harus di depan dan semua angka/nama/tanggal/status harus didukung sumber. Jika sumber tidak cocok dengan karakter tata letak, sederhanakan struktur tetapi jangan menciptakan isi demi memenuhi template.`
    : '';
  const specialStructure = `${formatStructure} ${sourceStructure}`.trim();
  const layoutStructureInstruction = layoutInstruction(contentLayout);
  const layoutDirection = layoutStructureInstruction ? `${layoutStructureInstruction} Tata letak "${LAYOUT_LABELS[contentLayout]}" menentukan struktur dan gaya kalimat pada carousel ini. Abaikan struktur bawaan format lama bila bertentangan dengan tata letak. Jangan mengubah inti topik atau mengarang fakta demi memenuhi template.` : '';
  ...
  if (contentLayout !== 'default') content.contentLayout = contentLayout;
  if (writingFormat === 'Masalah dan solusi') content.body = normalizeLegacySolutionBody(content.body);
  if (content.slides !== undefined) content.slides = writingFormat === 'Masalah dan solusi' ? normalizeProblemSolutionSlides(content.slides) : normalizeSlides(content.slides);
  const validationContent = value => value.slides === undefined
    ? { ...value, slides: writingFormat === 'Masalah dan solusi' ? normalizeProblemSolutionSlides(legacyProblemSolutionSlides(value)) : legacySlides(value) }
    : value;
  ...
  const validateGeneratedContent = value => {
    ...
    const result = validateContent(normalized, { format: writingFormat, manualTopic: value.slides === undefined ? '' : manualTopic, validateCopy: !options.useSources && options.skipCopyValidation !== true });
    result.push(...validateLayoutSlides(contentLayout, normalized.slides));
  ...
  if (errors.length) {
    ...
    if (options.useSources) {
      ...
      const fallbackErrors = [...validateContent(content, { format: writingFormat, manualTopic, validateCopy: false }), ...validateLayoutSlides(contentLayout, content.slides), ...validateSourceGrounding(content, options.sourceContext, options.sources)];
