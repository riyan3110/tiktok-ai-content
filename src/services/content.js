...
const { resolveContentLayout, layoutInstruction } = require('./contentLayouts');
...
async function generateContent(previousTopics, options = {}, client) {
  ...
  const layoutDirection = layoutStructureInstruction ? `${layoutStructureInstruction} Tata letak "${LAYOUT_LABELS[contentLayout]}" menentukan struktur slide; format "${format}" tetap mengatur gaya penyajian. Jangan mengubah inti topik.` : '';
