...
function createSiteAuthGateway(innerApp, config) {
  ...
  const sendAppShell = async (req, res, next) => {
    try {
      ...
      const finalLayoutScript = '<script defer src="/neo-layout-final.js?v=neo-layout-final-20260826c"></script>';
      const providerMobileHostFixScript = '<script defer src="/provider-mobile-host-fix.js?v=provider-host-20260826a"></script>';
