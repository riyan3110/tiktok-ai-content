...
function semanticAuditPrompt(content, topic, format = '') {
  ...
  const coherenceRules = String(format || '').toLocaleLowerCase('id-ID') === 'masalah dan solusi'
    ? `\n6. Baca SLIDES sebagai satu alur. Setiap SOLUSI/tips harus langsung menjawab MASALAH yang dibahas, bukan mengambil poin lain dari artikel hanya karena evidence-nya valid;\n7. title, body, dan points dalam satu slide harus menjelaskan satu ide yang sama. Tandai field yang memasukkan ide berbeda atau bullet yang menjadi topik baru;\n8. SOLUSI lanjutan dan HASIL/PENUTUP harus tetap mengikuti fokus masalah dan solusi sebelumnya. Tandai outcome yang tidak didukung atau hanya mengulang outcome lain;\n9. Jangan mengubah kemungkinan menjadi kepastian. Jika evidence memakai can/could/may/help/potential atau dapat/bisa/berpotensi/membantu, tandai claim pasti seperti “meningkat”, “menghasilkan”, atau “pasti lebih baik”;\n10. Nilai hubungan antarklaim memakai konteks SLIDES, tetapi setiap field tetap harus dibuktikan oleh satu evidence miliknya. Jangan menggabungkan dua evidence menjadi kesimpulan baru.`
    : '';
  return `Anda auditor entailment fakta bilingual Indonesia-Inggris. Gunakan HANYA claim, evidence, dan konteks SLIDES yang diberikan. Jangan memakai pengetahuan luar.\n\nTOPIK UTAMA: ${JSON.stringify(topic || '')}\nFORMAT: ${JSON.stringify(format || '')}\nSLIDES: ${JSON.stringify(slides)}\nCLAIMS: ${JSON.stringify(claims)}\n\nUntuk setiap claim, tandai unsupported bila salah satu berlaku:\n1. evidence tidak menyatakan makna yang sama dengan claim.text;\n2. claim menambahkan tujuan, sebab-akibat, manfaat, aplikasi, risiko, keselamatan, masa depan, atau kesimpulan yang tidak dinyatakan evidence;\n3. claim bertentangan dengan evidence, termasuk menyebut sesuatu sederhana ketika evidence menyebutnya sulit;\n4. claim mungkin benar secara umum tetapi evidence yang dipasangkan tidak membuktikannya;\n5. claim/evidence menyimpang jauh dari TOPIK UTAMA dan hanya mengambil side-note artikel yang tidak membantu menjawab topik.${coherenceRules}\n\nTerjemahan/parafrase Indonesia dari evidence Inggris boleh dan harus dianggap supported bila maknanya setia. Jangan menolak hanya karena beda bahasa.\nKembalikan HANYA JSON {"unsupported":[{"field":"slide:0:body","reason":"alasan singkat"}]}. Jika semua didukung, kembalikan {"unsupported":[]}.`;
}
