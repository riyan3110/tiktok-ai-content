...
const { resolveContentLayout, contentFormatForLayout, layoutInstruction } = require('./contentLayouts');
...
function validateVerifiedContent(base, candidate, { contentService, format, manualTopic, sources, autoSourceTopic = false }) {
  ...
  candidate.slides.forEach((slide, index) => {
    ...
    for (const field of slideFields(slide, index)) {
      if (/lanjut\s+baca\s+tentang/i.test(field.value)) errors.push(`${field.key}: memakai filler "Lanjut baca tentang".`);
      if (BOILERPLATE.test(field.value)) errors.push(`${field.key}: metadata/boilerplate website masuk ke konten.`);
    }
  });
...
function semanticAuditPrompt(content, topic, format = '') {
  ...
  const coherenceRules = String(format || '').toLocaleLowerCase('id-ID') === 'masalah dan solusi'
    ? `\n13. Baca SLIDES sebagai satu alur. Setiap SOLUSI/tips harus langsung menjawab MASALAH yang dibahas, bukan mengambil poin lain dari artikel hanya karena evidence-nya valid;\n14. title, body, dan points dalam satu slide harus menjelaskan satu ide yang sama. Tandai field yang memasukkan ide berbeda atau bullet yang menjadi topik baru;\n15. SOLUSI lanjutan dan HASIL/PENUTUP harus tetap mengikuti fokus masalah dan solusi sebelumnya. Tandai outcome yang tidak didukung atau hanya mengulang outcome lain;\n16. Jangan mengubah kemungkinan menjadi kepastian. Jika evidence memakai can/could/may/help/potential atau dapat/bisa/berpotensi/membantu, tandai claim pasti seperti “meningkat”, “menghasilkan”, atau “pasti lebih baik”;\n17. Nilai hubungan antarklaim memakai konteks SLIDES, tetapi setiap field tetap harus dibuktikan oleh satu evidence miliknya. Jangan menggabungkan dua evidence menjadi kesimpulan baru.`
    : '';
  return `Anda auditor entailment fakta bilingual Indonesia-Inggris. Gunakan HANYA claim, evidence, dan konteks SLIDES yang diberikan. Jangan memakai pengetahuan luar.\n\nTOPIK UTAMA: ${JSON.stringify(topic || '')}\nFORMAT: ${JSON.stringify(format || '')}\nSLIDES: ${JSON.stringify(slides)}\nCLAIMS: ${JSON.stringify(claims)}\n\nUntuk setiap claim, tandai unsupported bila salah satu berlaku:\n1. evidence tidak menyatakan makna yang sama dengan claim.text;\n2. claim menambahkan tujuan, sebab-akibat, manfaat, aplikasi, risiko, keselamatan, masa depan, atau kesimpulan yang tidak dinyatakan evidence;\n3. claim bertentangan dengan evidence, termasuk menyebut sesuatu sederhana ketika evidence menyebutnya sulit;\n4. claim mungkin benar secara umum tetapi evidence yang dipasangkan tidak membuktikannya;\n5. claim/evidence menyimpang jauh dari TOPIK UTAMA dan hanya mengambil side-note artikel yang tidak membantu menjawab topik;\n6. ENTITY TYPE berubah: tool/alat, model, aplikasi, fitur, produk, perusahaan, dan layanan adalah jenis berbeda dan tidak boleh saling ditukar;\n7. NAMED LIST atau SCOPE berubah: pertahankan persis secara makna daftar paket/tier, platform, wilayah, model, atau kelompok pengguna. Menambah, menghapus, atau mengganti anggota daftar adalah unsupported;\n8. MODALITY lebih kuat: can/could/may/help/designed to/intended to harus tetap dapat/bisa/mungkin/membantu/dirancang untuk, bukan pasti/selalu/menjamin/mencegah/menghilangkan;\n9. NEGATION atau EXCLUSION hilang/berbalik, termasuk ketidaktersediaan pada tier tertentu;\n10. TEMPORAL FACT bergeser: tanggal, bulan, tahun, urutan rollout, preview, default, availability, atau perubahan setting harus tetap sama;\n11. NESTED UNCERTAINTY / UNCERTAINTY ABOUT CAPABILITY hilang: pertahankan seluruh uncertainty scope, bukan hanya modal verb di klausa dalam. Wrapper seperti cannot rule out, cannot exclude, possibility that, possible that, potentially, risk that, may eventually, could potentially, concern that, suspected to, expected to (bila masih proyeksi), belum dapat mengesampingkan kemungkinan, ada kemungkinan, berpotensi, diperkirakan, atau dikhawatirkan dapat tidak boleh dipadatkan menjadi X can/dapat/mampu Y;\n12. UNSUPPORTED CONDITION / CONDITIONAL FRAME ditambahkan: kondisi seperti jika dirilis, jika digunakan publik, ketika tersedia luas, atau setelah diluncurkan harus dinyatakan evidence; jika tidak, tandai unsupported.${coherenceRules}\n\nPeriksa detail-detail ini walaupun tema umum claim benar. Terjemahan/parafrase Indonesia dari evidence Inggris boleh dan harus dianggap supported bila entity type, scope, uncertainty scope, kondisi, modalitas, negasi, serta waktu tetap setia. Jangan menolak hanya karena beda bahasa.\nKembalikan HANYA JSON {"unsupported":[{"field":"slide:0:body","reason":"alasan singkat"}]}. Jika semua didukung, kembalikan {"unsupported":[]}.`;
}
...
function verifierPrompt({ base, draft = base, bank, topic, format, errors = [], autoSourceTopic = false, contentLayout = 'default' }) {
  ...
  const factFormatRules = layout === 'default' && String(format || '').toLocaleLowerCase('id-ID') === 'fakta singkat'
...
async function generateFilteredContent({ content, previousTopics = [], options = {}, sources = [], client }) {
  ...
  let base = await content.generateContent(previousTopics, {
    ...
    deferSourceGroundingValidation: autoSourceTopic,
  ...
  for (let attempt = 1; attempt <= MAX_VERIFY_ATTEMPTS; attempt += 1) {
    const response = await openai.chat.completions.create({
      model: config.aiModel,
      messages: [
        { role: 'system', content: `Anda memfilter fakta carousel. Tata letak user adalah "${contentLayout}" dan tidak boleh diganti. Sumber hanya untuk verifikasi fakta.` },
        { role: 'user', content: verifierPrompt({ base, draft, bank, topic, format: validationFormat, errors, autoSourceTopic, contentLayout }) }
      ],
      response_format: { type: 'json_object' }
    });
    let candidate;
  ...
  while (safeRecoveryAttempts < MAX_SAFE_RECOVERY_ATTEMPTS) {
    ...
    const response = await openai.chat.completions.create({
      model: config.aiModel,
      messages: [
        { role: 'system', content: `Anda melakukan recovery field secara ketat. Tata letak user adalah "${contentLayout}". Jangan mengarang dan jangan mengubah struktur carousel.` },
        { role: 'user', content: safeRecoveryPrompt({ base, draft, bank, topic, format: validationFormat, errors, autoSourceTopic, contentLayout }) }
      ],
      response_format: { type: 'json_object' }
    });
    let candidate;
  ...
  // A provider can repeatedly fail to ground a factual title even when the
  // substantive body/points are sound. Only after targeted recovery is
  // exhausted, replace that title with a non-factual structural label and run
  // all strict gates again.
  ...
  if (autoSourceTopic && titleOnly && titleGroundingFailure) {
    ...
    const slides = draft.slides.map((slide, slideIndex) => {
      ...
      if (!remainingFields.has(field)) return slide;
      return {
        ...slide,
        title: labels[Math.min(slideIndex, labels.length - 1)],
        claims: (slide.claims || []).filter(claim => claim?.field !== field)
      };
    });
