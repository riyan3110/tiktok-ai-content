...
const GROUNDING_STOPWORDS = new Set([
  ...TOPIC_STOPWORDS,
  'agar', 'karena', 'saat', 'ketika', 'oleh', 'dalam', 'lebih', 'juga', 'akan', 'bisa', 'dapat',
  'sebelum', 'setelah', 'mereka', 'kami', 'kamu', 'anda', 'tersebut', 'sebuah', 'para', 'jadi'
]);
...
function requiresEvidence(value, section = '', kind = 'body') {
  ...
  // themselves. Do not turn them into hard failures merely because they use a
  // verb such as "membantu" or a noun such as "fitur/temuan".
  if (NON_FACTUAL_START.test(text)) return false;
  if (isQuestion(text)) return false;

  if (FACT_VERBS.test(text)) return true;
  // A marker-only list point is commonly a short label, not a factual claim.
  // Titles/body can still be factual noun phrases and therefore stay guarded.
  if (kind !== 'point' && FACT_MARKERS.test(text)) return true;
  return false;
}
  ...
  candidate.slides.forEach((slide, slideIndex) => {
    for (const field of slideFields(slide, slideIndex)) {
      ...
      for (const item of evidence) {
        ...
        const acceptable = support.exact || (support.total >= 2 && support.matches >= 2 && support.score >= 0.65);
...
function validateManualTopicIdentity(manualTopic, slides = []) {
  ...
  // specific name/product is present, validate that the carousel keeps at least
  // one token from that identity instead of demanding every token literally.
  // Example: "Google Maps" may naturally be shortened to "Maps".
...
function validateVerifiedContent(base, candidate, { contentService, format, manualTopic, sources }) {
  ...
  // The verifier occasionally returns a factual field without its claim even
  // when the same fact exists in the supplied source. Recover only when there
  // is strong deterministic lexical support; unsupported copy still fails.
...
async function generateFilteredContent({ content, previousTopics = [], options = {}, sources = [], client }) {
  // The old manual-topic validator is a literal token-overlap gate. For source
  // filtering, keep the requested topic in the generation prompt but bypass
  // that gate; entity identity is validated after source verification instead.
