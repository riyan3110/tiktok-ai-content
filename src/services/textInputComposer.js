...
function validateResult(result, sourceText, requestedSlideCount = targetSlideCount(sourceText), contentLayout = 'default') {
  ...
  errors.push(...validateLayoutSlides(contentLayout, slides));
...
function promptFor(text, requestedSlideCount, contentLayout = 'default') {
  ...
  const selectedSections = layoutSections(layout, requestedSlideCount);
  const effectiveSections = selectedSections || (requestedSlideCount === 5
    ? ['HOOK', 'FAKTA UTAMA', 'DETAIL', 'KONTEKS', 'PENUTUP']
    : ['HOOK', 'FAKTA UTAMA', 'DETAIL', 'PENUTUP']);
  const optionalContext = requestedSlideCount === 5
    ? `- Slide 4 = ${effectiveSections[3]}. Body 10–16 kata dan points wajib [].\n`
    : '';
  const selectedLayoutInstruction = layout === 'default'
    ? ''
    : `\nTATA LETAK TERPILIH: ${layout.toUpperCase()}\n${layoutInstruction(layout)}\nSection WAJIB berurutan: ${effectiveSections.join(' → ')}.\n`;

  return `MODE: GENERATE DARI TEKS — TRANSFORM ONLY.${selectedLayoutInstruction}
...
- Slide 1 = ${effectiveSections[0]}. HANYA judul besar 7–10 kata yang padat, jelas, dan menarik tanpa clickbait berlebihan. body wajib "" dan points wajib [].
- Slide 2 = ${effectiveSections[1]}. Body 8–14 kata + 2–3 bullet. Setiap bullet 3–7 kata dan membawa informasi berbeda.
- Slide 3 = ${effectiveSections[2]}. Body 8–14 kata + 2–3 bullet. Setiap bullet 3–7 kata. Jangan mengulang slide 2.
${optionalContext}- Slide terakhir = ${effectiveSections.at(-1)}. Body 14–18 kata. points wajib []. Jangan sekadar mengulang hook.
...
- Hindari tautologi dan pengulangan makna dalam satu field.
...
- Judul besar WAJIB spesifik pada objek/fakta dari TEXT_INPUT.
- Satu fakta utama cukup muncul sekali.
- Judul adalah angle spesifik slide, bukan salinan body.
- Bullet harus singkat, natural, spesifik, dan bersumber dari TEXT_INPUT.
- Bullet tidak boleh mengulang body atau bullet lain pada slide yang sama.
- Nama media/publisher yang hanya menjadi sumber berita jangan dijadikan bullet.
- Semua judul antar-slide harus berbeda.
- Penutup harus mengikuti kesimpulan TEXT_INPUT tanpa menaikkan tingkat kepastian.
...
- Caption WAJIB ${CAPTION_MIN_WORDS}–${CAPTION_MAX_WORDS} kata, 1–2 kalimat.
...
{"topic":"judul/topik singkat","caption":"...","hashtags":["#..."],"slides":[{"section":"${effectiveSections[0]}","title":"...","body":"","points":[]},{"section":"${effectiveSections[1]}","title":"...","body":"...","points":["...","..."]},{"section":"${effectiveSections[2]}","title":"...","body":"...","points":["...","..."]}${requestedSlideCount === 5 ? `,{"section":"${effectiveSections[3]}","title":"...","body":"...","points":[]}` : ''},{"section":"${effectiveSections.at(-1)}","title":"...","body":"...","points":[]}]} `;
}
...
async function compose({ text, client, contentLayout = 'default' } = {}) {
  ...
  const messages = [
    ...
    { role: 'user', content: promptFor(sourceText, requestedSlideCount, contentLayout) }
  ];
  ...
  for (let repair = 0; repair <= MAX_REPAIRS; repair += 1) {
    const checked = validateResult(parsed, sourceText, requestedSlideCount, contentLayout);
