...
function duplicateSlideCopy(slide = {}) {
  const fields = [slide.title, slide.body, ...(slide.points || [])]
    .map(value => {
      ...
      const text = normalized(raw).replace(/[.,:;!?]/g, ' ').replace(/\s+/g, ' ').trim();
      return { raw, text, wordCount: words(text).length };
    })
  ...
  for (let left = 0; left < fields.length; left += 1) {
    for (let right = left + 1; right < fields.length; right += 1) {
      ...
      if (shorter.wordCount >= 4 && longer.text.includes(shorter.text)) return true;
    }
  }
  return false;
}
