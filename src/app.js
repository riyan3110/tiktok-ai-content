...
const sourceFetcherService = require('./services/sourceFetcher');
...
const normalizeContentStudioInput = (body, provider = body?.provider) => provider === 'vidu'
  ? { ...body, aspectRatio: '9:16', resolution: '1080p', parameters: { ...body.parameters, aspectRatio: '9:16', resolution: '1080p' } }
  : body;
const templateService = require('./services/templates');
const { StorageService } = require('./storage/service');
const { ContentStudioService } = require('./services/contentStudio');
const { OrcaRouterModels } = require('./services/orcaRouterModels');
const { NineRouterModels } = require('./services/nineRouterModels');
...
function createApp({ db, content = contentService, images = imageService, tiktok = tiktokService, trending = trendingService, sourceFetcher = sourceFetcherService, manualSourceRoleGuard = null, automation = automationService, aiTransport, storageTransport } = {}) {
  ...
  const textRuntime = require('./services/textProviderRuntime');
  textRuntime.bind(db, aiTransport);
  app.use(textRuntime.middleware(db, aiTransport));
  ...
  const backgroundQueues = new Map(); const backgroundLatest = new Map();
  const studio = new ContentStudioService({ db, storage });
  const orcaRouterModels = new OrcaRouterModels({ db, connector: aiConnector, transport: aiTransport || fetch });
  const nineRouterModels = new NineRouterModels({ db, connector: aiConnector, transport: aiTransport || fetch });
  ...
  app.get('/api/ai/providers/9router/models', async (req, res, next) => { try { res.json(await nineRouterModels.get({ refresh: req.query.refresh === 'true' })); } catch (e) { next(e); } });
  ...
  app.post('/api/content-studio/generate', async (req, res, next) => { try { req.body = { ...req.body, mediaType: req.body?.mediaType || 'image' }; let validation = aiConnector.validateGeneration(db, req.body); req.body = normalizeContentStudioInput({ ...req.body, provider: validation.provider, model: validation.dynamic ? validation.row.default_model : req.body.model }, validation.provider); if (req.body?.provider === '9router') { const mediaType = req.body.mediaType === 'video' ? 'video' : 'image'; const available = await nineRouterModels.get(); const group = available[mediaType]; const valid = [...group.combos, ...group.directModels]; if (!req.body.model || !valid.includes(req.body.model)) throw Object.assign(new Error(`Model 9Router ${mediaType} tidak valid`), { status: 422 }); req.body.modelType = group.combos.includes(req.body.model) ? 'combo' : 'direct'; req.body.capability = mediaType; } if (req.body?.provider === 'orcarouter') { const mediaType = req.body.mediaType === 'video' ? 'video' : 'image'; const available = await orcaRouterModels.get(); if (!req.body.model || req.body.model === 'orcarouter/auto' || !available[mediaType].includes(req.body.model)) throw Object.assign(new Error(`Model OrcaRouter ${mediaType} tidak valid`), { status: 422 }); } validation = aiConnector.validateGeneration(db, req.body); const count = validation.count; const ids = []; const selected = req.body?.assetIds?.length ? await storage.resolveIds(req.body.assetIds) : (req.body?.assets || []); const assets = selected.map(asset => ({ id: asset.id, type: asset.type === 'video' ? 'video' : 'image', name: asset.name, mimeType: asset.mime_type || asset.mimeType, url: asset.url })); aiConnector.validateGeneration(db, { ...req.body, provider: validation.provider, assets }); for (let index = 0; index < count; index += 1) { const id = crypto.randomUUID(); const job = { ...req.body, provider: validation.provider, id, assets, count, metadata: { negativePrompt: req.body?.negativePrompt || '', resolution: req.body?.resolution || '', batchIndex: index + 1 } }; studio.createQueued(id, job); ids.push(id); mediaWorker.enqueue(job); } res.status(202).json({ ids, status: 'Queued', provider: validation.provider }); } catch (e) { next(e); } });
  app.post('/api/content-studio/jobs/:id/retry', (req, res, next) => { try { const old = studio.get(req.params.id); if (!old) return res.status(404).json({ error: 'Job tidak ditemukan' }); const body = normalizeContentStudioInput({ provider: old.provider, model: old.model || undefined, prompt: old.prompt, negativePrompt: old.negative_prompt, mediaType: old.media_type, assets: old.assets, resolution: old.resolution, metadata: { ...old.metadata, retryOf: old.id } }); const validation = aiConnector.validateGeneration(db, body); const id = crypto.randomUUID(); studio.createQueued(id, { ...body, provider: validation.provider }); mediaWorker.enqueue({ ...body, id, provider: validation.provider }); res.status(202).json({ id, status: 'Queued' }); } catch (e) { next(e); } });
  ...
  const providersResponse = () => aiConnector.configuredProviders(db).map(row => row.roles ? {
    provider: row.provider, name: row.name, capabilities: row.roles, baseUrl: row.base_url,
    defaultModel: row.default_model, textModel: row.text_model, imageModel: row.image_model,
    defaultCapabilities: row.defaults, enabled: true, hasApiKey: true
  } : aiConnector.publicSetting(row, aiConnector.defaultCapabilities(db, row.provider)));
  ...
  app.get('/api/ai/providers', (req, res) => res.json(providersResponse()));
  app.put('/api/ai/providers/:provider', async (req, res, next) => { try { const body = { ...(req.body || {}) }; if (req.params.provider === '9router' && body.isDefault) { const capability = body.defaultCapability || 'text'; const models = await nineRouterModels.get(); const field = `${capability}Model`; const row = aiConnector.setting(db, '9router'); if (!body[field] && !row[`${capability}_model`]) body[field] = models[capability]?.combos[0] || models[capability]?.directModels[0]; } res.json(aiConnector.save(db, req.params.provider, body)); } catch (e) { next(e); } });
  ...
  app.post('/api/ai/providers/:provider/test', async (req, res, next) => { const started = Date.now(); try { const row = aiConnector.setting(db, req.params.provider); if (!row.enabled) throw Object.assign(new Error('Provider is disabled'), { status: 409 }); if (!row.api_key_encrypted) throw Object.assign(new Error('API key is required'), { status: 422 }); const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), row.timeout_ms); try { const discovered = req.params.provider === '9router' ? await nineRouterModels.get({ refresh: true }) : null; const result = discovered ? { connected: true, responseTime: Date.now() - started, providerVersion: 'Available', counts: discovered.counts, capabilities: discovered.capabilities } : await ProviderFactory.create(aiConnector.configured(row), aiTransport).testConnection({ signal: controller.signal }); aiConnector.updateHealth(db, row.provider, true, result); res.json(result); } finally { clearTimeout(timer); } } catch (caught) { const error = normalizeError(caught); aiConnector.updateHealth(db, req.params.provider, false, { responseTime: Date.now() - started }); next(error); } });
  ...
  app.post('/api/templates/:id/generate', (req, res, next) => { try {
    const item = templateService.get(db, req.params.id); if (item.target_ai === 'video') aiConnector.setting(db, item.provider); else if (!require('./services/dynamicAiProviders').selectedProfile(db, item.target_ai === 'text' ? 'text' : 'image')) throw Object.assign(new Error('Default provider belum dipilih.'), { status: 409 });
  ...
  app.post('/generate', async (req, res, next) => { try { const watermark = typeof req.body?.watermarkEnabled === 'boolean' ? { enabled: req.body.watermarkEnabled } : req.body?.watermark; const rawBackground = req.body?.background || {}; const clean = value => ({ type: value?.type === 'image' ? 'image' : 'color', color: /^#[0-9a-f]{6}$/i.test(value?.color || '') ? value.color : '#0B0B0D', assetId: value?.type === 'image' ? String(value.assetId || '') || null : null, previewUrl: value?.type === 'image' ? String(value.previewUrl || '') || null : null, textColor: value?.textColor === '#000000' ? '#000000' : '#FFFFFF' }); const uploaded = rawBackground.uploadedBackground?.assetId ? clean({ ...rawBackground.uploadedBackground, type: 'image' }) : null; const background = { ...clean(rawBackground), applyToAllSlides: rawBackground.applyToAllSlides !== false, slideBackgrounds: Object.fromEntries(Object.entries(rawBackground.slideBackgrounds || {}).slice(0, 5).map(([key, value]) => [key, clean(value)])), uploadedBackground: uploaded }; if (uploaded) { const asset = storage.repository.get(uploaded.assetId); if (!asset || !String(asset.mime_type).startsWith('image/')) throw Object.assign(new Error('Asset background tidak valid'), { status: 422 }); } for (const value of [background, ...Object.values(background.slideBackgrounds)]) { if (value.type === 'image' && value.assetId) { const asset = storage.repository.get(value.assetId); if (!asset || !String(asset.mime_type).startsWith('image/')) throw Object.assign(new Error('Asset background tidak valid'), { status: 422 }); const file = await storage.preview(asset); value.imageData = `data:${file.mimeType};base64,${file.data.toString('base64')}`; } } const id = await generateAndSave({ db, content, images, trending, sourceFetcher, manualSourceRoleGuard, useTrendReference: req.body?.useTrendReference !== false, forceNewAngle: req.body?.forceNewAngle === true, mode: req.body?.topicSource || 'ai', requestedTopic: req.body?.requestedTopic, category: req.body?.contentCategory || 'Iklan & UGC', customCategory: req.body?.customCategory, format: req.body?.contentFormat || 'Tutorial langkah', watermark, background, useSources: req.body?.useSources === true, sourceUrls: Array.isArray(req.body?.sourceUrls) ? req.body.sourceUrls : [] }); res.json(record(db, id)); } catch (e) { next(e); } });
  app.patch('/contents/:id/background', async (req, res, next) => {
    const contentId = Number(req.params.id); const revision = Number(req.body?.revision || Date.now());
    backgroundLatest.set(contentId, Math.max(backgroundLatest.get(contentId) || 0, revision));
    const previousTask = backgroundQueues.get(contentId) || Promise.resolve(); let release;
    const task = new Promise(resolve => { release = resolve; }); const queued = previousTask.then(() => task); backgroundQueues.set(contentId, queued);
    await previousTask;
    try {
      if (revision < backgroundLatest.get(contentId)) return res.status(409).json({ error: 'Permintaan preview sudah digantikan pilihan terbaru.', stale: true });
      ...
      const raw = req.body?.background || {}; const clean = value => ({ type: value?.type === 'image' ? 'image' : 'color', color: /^#[0-9a-f]{6}$/i.test(value?.color || '') ? value.color : '#0B0B0D', assetId: value?.type === 'image' ? String(value.assetId || '') || null : null, previewUrl: value?.type === 'image' ? String(value.previewUrl || '') || null : null, textColor: value?.textColor === '#000000' ? '#000000' : '#FFFFFF' });
      const uploadedBackground = raw.uploadedBackground?.assetId ? clean({ ...raw.uploadedBackground, type: 'image' }) : null;
      const background = { ...clean(raw), applyToAllSlides: raw.applyToAllSlides !== false, slideBackgrounds: Object.fromEntries(Object.entries(raw.slideBackgrounds || {}).slice(0, 5).map(([key, value]) => [key, clean(value)])), uploadedBackground };
      const saved = { ...background, uploadedBackground: uploadedBackground ? { ...uploadedBackground } : null };
      const source = JSON.parse(item.render_source || '{}');
      if (!source.topic) { db.prepare('UPDATE contents SET background=?,background_revision=? WHERE id=?').run(JSON.stringify(saved), revision, item.id); return res.status(409).json({ error: 'Konten lama harus dibuat ulang agar background export sama persis.', legacy: true }); }
      for (const value of [uploadedBackground, background, ...Object.values(background.slideBackgrounds)].filter(Boolean)) { if (value.type === 'image' && value.assetId) { const asset = storage.repository.get(value.assetId); if (!asset || !String(asset.mime_type).startsWith('image/')) throw Object.assign(new Error('Asset background tidak valid'), { status: 422 }); if (value !== uploadedBackground || background.type === 'image') { const file = await storage.preview(asset); value.imageData = `data:${file.mimeType};base64,${file.data.toString('base64')}`; } } }
      let temporary = [];
      try {
        temporary = await images.createSlides(`pending-${item.id}-${revision}-${Math.random().toString(36).slice(2, 8)}`, { ...source, background });
        if (revision < backgroundLatest.get(contentId)) { if (images.cleanupSlides) await images.cleanupSlides(temporary); return res.status(409).json({ error: 'Permintaan preview sudah digantikan pilihan terbaru.', stale: true }); }
        const cleanSaved = { ...saved, imageData: undefined, slideBackgrounds: Object.fromEntries(Object.entries(background.slideBackgrounds).map(([key, value]) => [key, { ...value, imageData: undefined }])) };
        const save = stable => db.prepare('UPDATE contents SET slides=?,background=?,background_revision=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(JSON.stringify(stable), JSON.stringify(cleanSaved), revision, item.id);
        if (images.promoteSlides) await images.promoteSlides(temporary, item.id, JSON.parse(item.slides || '[]'), save); else save(temporary);
      } catch (error) { if (images.cleanupSlides) await images.cleanupSlides(temporary); throw error; }
      res.json(record(db, item.id));
    } catch (e) { next(e); } finally { release(); if (backgroundQueues.get(contentId) === queued) backgroundQueues.delete(contentId); }
  });
