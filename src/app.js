...
function createApp({ db, content = contentService, images = imageService, tiktok = tiktokService, trending = trendingService, sourceFetcher = sourceFetcherService, automation = automationService, aiTransport, storageTransport } = {}) {
  ...
  const providersResponse = () => { aiConnector.seed(db); const health = new Map(db.prepare('SELECT * FROM ai_provider_health').all().map(x => [x.provider, x])); return ProviderFactory.names().map(provider => db.prepare('SELECT * FROM ai_provider_settings WHERE provider=?').get(provider)).map(row => ({ ...aiConnector.publicSetting(row, aiConnector.defaultCapabilities(db, row.provider)), health: health.get(row.provider) || { status: 'Offline', quota_status: 'Unknown' } })); };
