...
function createApp({ db, content = contentService, images = imageService, tiktok = tiktokService, trending = trendingService, automation = automationService, aiTransport, storageTransport } = {}) {
  ...
  app.put('/api/ai/providers/:provider', async (req, res, next) => { try { const body = { ...(req.body || {}) }; if (req.params.provider === '9router' && body.isDefault) { const capability = body.defaultCapability || 'text'; const models = await nineRouterModels.get(); if (!models[capability]?.length) throw Object.assign(new Error(`9Router tidak memiliki model ${capability} yang terdeteksi: ${(models.diagnostics?.image?.reasons || []).join('; ')}`), { status: 409 }); const field = `${capability}Model`; const row = aiConnector.setting(db, '9router'); if (!body[field] && !row[`${capability}_model`]) body[field] = models[capability][0]; } res.json(aiConnector.save(db, req.params.provider, body)); } catch (e) { next(e); } });
