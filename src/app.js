...
function createApp({ db, content = contentService, images = imageService, tiktok = tiktokService, trending = trendingService, automation = automationService, aiTransport, storageTransport } = {}) {
  ...
  app.patch('/contents/:id/background', async (req, res, next) => { try {
    const item = db.prepare('SELECT * FROM contents WHERE id=?').get(Number(req.params.id));
    if (!item) return res.status(404).json({ error: 'Konten tidak ditemukan' });
    const source = JSON.parse(item.render_source || '{}');
    if (!source.topic) throw Object.assign(new Error('Konten lama tidak memiliki sumber render dan perlu dibuat ulang.'), { status: 409 });
    ...
    const slides = await images.createSlides(`pending-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, { ...source, background });
    const saved = { ...background, imageData: undefined, uploadedBackground: uploadedBackground ? { ...uploadedBackground, imageData: undefined } : null, slideBackgrounds: Object.fromEntries(Object.entries(background.slideBackgrounds).map(([key, value]) => [key, { ...value, imageData: undefined }])) };
    db.prepare('UPDATE contents SET slides=?,background=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(JSON.stringify(slides), JSON.stringify(saved), item.id);
