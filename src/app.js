...
const normalizeContentStudioInput = body => body;
...
function createApp({ db, content = contentService, images = imageService, tiktok = tiktokService, trending = trendingService, sourceFetcher = sourceFetcherService, manualSourceRoleGuard = null, automation = automationService, aiTransport, storageTransport } = {}) {
  ...
  require('./services/providerStorageCleanup').run(db, db.name === ':memory:' ? { keyFile: null } : {});
  ...
  const providersResponse = () => aiConnector.configuredProviders(db).map(row => ({
  ...
  function testProvider(provider, res) { return res.status(410).json({ error: 'Gunakan Provider AI manual untuk memvalidasi Base URL dan API key.' }); }
  app.get('/api/ai/providers', (req, res) => res.json(providersResponse()));
  app.all('/api/ai/providers/:provider/:action', (req, res) => res.status(410).json({ error: 'Endpoint provider lama sudah dihapus.' }));
  app.put('/api/ai/providers/:provider', (req, res) => res.status(410).json({ error: 'Gunakan Provider AI manual.' }));
