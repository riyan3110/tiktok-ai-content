...
function createApp({ db, content = contentService, images = imageService, tiktok = tiktokService, trending = trendingService } = {}) {
  ...
  app.post('/generate', async (req, res, next) => { try { const id = await generateAndSave({ db, content, images, trending, mode: req.body?.topicSource || 'ai', requestedTopic: req.body?.requestedTopic }); res.json(record(db, id)); } catch (e) { next(e); } });
