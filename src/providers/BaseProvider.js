...
class BaseProvider {
  ...
  parse(data) { const media = data.media || data.images || data.videos || (data.url ? [{ url: data.url }] : []); return { content: data.output || data.text || '', media: Array.isArray(media) ? media : [media], providerJobId: data.id || data.job_id || data.task_id || null, status: data.status, usage: data.usage || {}, raw: data }; }
