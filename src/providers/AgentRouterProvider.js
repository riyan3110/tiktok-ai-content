...
class AgentRouterProvider extends BaseProvider {
  ...
  buildResponsesRequest(input = {}, probe = false) {
    const body = {
    ...
    if (maxTokens > 0) body.max_output_tokens = Math.max(1, Math.min(8192, maxTokens));
    return body;
  }
    ...
    if (Array.isArray(data.output)) {
      for (const item of data.output) {
        ...
        for (const part of item.content) {
          if (typeof part?.text === 'string' && (!part.type || ['output_text', 'text'].includes(part.type))) outputParts.push(part.text);
        }
      }
    }
    if (!outputParts.length && data.choices?.[0]?.message?.content) outputParts.push(data.choices[0].message.content);
    if (!outputParts.length && Array.isArray(data.content)) {
      outputParts.push(...data.content.filter(item => item?.type === 'text' && typeof item.text === 'string').map(item => item.text));
    }
    const content = outputParts.join('').trim();
    if (!content) throw Object.assign(new Error('AgentRouter Responses API tidak mengembalikan respons teks'), { type: 'Provider Error', code: 'EMPTY_TEXT_RESPONSE' });
  ...
  async executeFallback(input, signal) {
    ...
    try {
      ...
      try {
        const data = await this.requestJson(url, {
          ...
          body: JSON.stringify(this.buildResponsesRequest(input)),
          signal
        });
        result = this.parseResponses(data);
      } catch (primaryError) {
        if (![404, 405].includes(Number(primaryError.status || 0))) throw primaryError;
        result = await this.executeFallback(input, signal);
      }
  ...
  async testConnection({ signal } = {}) {
    ...
    try {
      let result;
      try {
        result = this.parseResponses(await this.requestJson(url, {
          method: 'POST',
          headers: this.openAiHeaders(),
          body: JSON.stringify(this.buildResponsesRequest({ model, prompt: 'Reply OK' }, true)),
          signal
        }));
      } catch (primaryError) {
        if (![404, 405].includes(Number(primaryError.status || 0))) throw primaryError;
        result = await this.executeFallback({ mediaType: 'text', model, prompt: 'Reply OK', parameters: { maxTokens: 1 } }, signal);
      }
