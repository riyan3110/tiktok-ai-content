...
test('real media response and metadata are persisted and exposed by result viewer API', async () => {
  ...
  await request(app).put('/api/ai/providers/runway').send({ apiKey: 'secret', enabled: true });
  const generated = await request(app).post('/api/ai/generations').send({ provider: 'runway', prompt: 'Create product ad', mediaType: 'image', assets: [{ type: 'reference-product', url: 'https://assets.example/item.png' }] });
...
test('batch endpoint accepts multiple jobs for background worker', async () => {
  const db = createDatabase(':memory:'); const app = createApp({ db, aiTransport: async () => response({ output: 'ok' }) }); await request(app).put('/api/ai/providers/openai').send({ apiKey: 'secret', enabled: true });
  const batch = await request(app).post('/api/ai/generations/batch').send({ jobs: [{ provider: 'openai', prompt: 'one' }, { provider: 'openai', prompt: 'two' }] }); assert.equal(batch.status, 202); assert.equal(batch.body.ids.length, 2);
});
