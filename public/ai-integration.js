(() => {
  ...
  function renderProviders() { $('#integration-providers').innerHTML = PROVIDERS.map(name => `<button type="button" data-integration-provider="${safe(name)}" class="integration-provider ${name === config.provider ? 'active' : ''}"><span>${safe(name.split(' ')[0].slice(0, 2).toUpperCase())}</span><b>${safe(name)}</b><i class="health-${safe((health[name] || 'Offline').toLowerCase())}">${ic('circle-dot')} ${safe(health[name] || 'Offline')}</i></button>`).join(''); document.querySelectorAll('[data-integration-provider]').forEach(button => button.onclick = () => { config.provider = button.dataset.integrationProvider; $('#integration-provider').value = config.provider; write(KEYS.config, config); renderProviders(); renderMonitor(); }); }
  ...
  async function run(event) {
    ...
    try {
      ...
      $('#integration-result').innerHTML = `<div><span>${ic('check')}</span><b>Normalized Response</b></div><p>${safe(response.content)}</p><small>${response.tokens.total} tokens · ${safe(response.finishReason)}</small><ul>${response.warnings.map(item => `<li>${safe(item)}</li>`).join('')}</ul>`;
  ...
  $('#integration-flow').innerHTML = PIPELINE.map((name, index) => `<div data-pipeline-stage="${safe(name)}"><span>${String(index + 1).padStart(2, '0')}</span><b>${safe(name)}</b></div>${index < PIPELINE.length - 1 ? `<i>${ic('chevron-down')}</i>` : ''}`).join('');
