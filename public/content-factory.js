(() => {
  const templates = [
    ['sparkles','Fakta Unik','Edukasi','Fakta mengejutkan yang mudah dibagikan','5–7 slide','30 dtk','TikTok, Instagram'],['terminal','Tutorial AI','AI','Panduan praktis memakai tools AI','7–10 slide','45 dtk','TikTok, YouTube'],['braces','Prompt AI','AI','Prompt siap pakai dengan contoh hasil','3–5 slide','30 dtk','TikTok, X'],['rows-3','Edukasi','Edukasi','Konsep kompleks menjadi mudah dipahami','5–10 slide','45 dtk','Semua platform'],['arrow-up-right','Tips Bisnis','Bisnis','Tips yang aplikatif untuk pertumbuhan','5–7 slide','30 dtk','TikTok, LinkedIn'],['megaphone','Marketing','Marketing','Strategi dan insight pemasaran modern','5–10 slide','45 dtk','Instagram, TikTok'],['bitcoin','Crypto','Finansial','Edukasi aset digital yang bertanggung jawab','5–7 slide','45 dtk','X, TikTok'],['cpu','Teknologi','Teknologi','Update dan penjelasan teknologi','5–7 slide','30 dtk','TikTok, X'],['feather','Storytelling','Kreatif','Narasi dengan konflik dan resolusi','7–10 slide','60 dtk','Semua platform'],['flame','Motivasi','Lifestyle','Pesan inspiratif yang autentik','3–5 slide','30 dtk','Instagram, TikTok'],['package','Produk','Commerce','Sorotan manfaat dan diferensiasi produk','5–7 slide','30 dtk','TikTok Shop'],['star','Review Produk','Commerce','Review berimbang berbasis pengalaman','5–7 slide','45 dtk','TikTok, YouTube'],['smile','UGC','Commerce','Naskah natural ala pengguna produk','3–5 slide','30 dtk','TikTok, Instagram'],['gallery-horizontal','Carousel TikTok','Carousel','Carousel vertikal dengan retention kuat','3–15 slide','—','TikTok'],['hash','Thread X','Social','Thread ringkas dengan alur kuat','5–10 post','—','X'],['instagram','Instagram Carousel','Carousel','Carousel visual yang save-worthy','3–15 slide','—','Instagram'],['facebook','Facebook Post','Social','Post komunitas yang mengundang diskusi','1 post','—','Facebook'],['youtube','YouTube Shorts Script','Video','Script cepat dengan beat per adegan','—','15–60 dtk','YouTube'],['music','TikTok Script','Video','Script native untuk retention TikTok','—','15–60 dtk','TikTok']
  ...
  function renderTemplates(query = '') {
    ...
    $('#factory-templates').innerHTML = templates.filter(item => `${item.name} ${item.category} ${item.platform}`.toLocaleLowerCase('id').includes(normalized)).map(item => `<button type="button" class="factory-template ${item.name === selected.name ? 'active' : ''}" data-template="${safe(item.name)}"><span class="factory-template-icon">${ic(item.icon)}</span><span class="factory-template-copy"><small>${safe(item.category)}</small><b>${safe(item.name)}</b><em>${safe(item.description)}</em><i><span>${ic('gallery-horizontal')} ${safe(item.slides)}</span><span>${ic('clock')} ${safe(item.duration)}</span></i><strong>${safe(item.platform)}</strong></span></button>`).join('');
  ...
  function selectTemplate(name) {
    ...
    $('#factory-selected-title').textContent = selected.name; $('#factory-selected-icon').innerHTML = ic(selected.icon); $('#factory-category').value = selected.name;
  ...
  function renderPreview() {
    ...
    if (activeTab === 'thumbnail') html = `<div class="factory-thumbnail"><span>${ic(selected.icon)}</span><small>${safe(item.template)}</small><strong>${safe(item.brief.topic)}</strong><em>AI ADS LAB</em></div>`;
  ...
  async function openAssets() {
    ...
    if (!chosen) return;
    assets = chosen;
    $('#factory-assets-list').innerHTML = assets.map(asset => `<span class="selected-asset"><img src="${safe(asset.previewUrl)}" alt=""><button type="button" data-remove-factory-asset="${safe(asset.id)}" aria-label="Remove asset">${ic('x')}</button></span>`).join('');
  ...
  document.querySelectorAll('[data-factory-export]').forEach(button => button.onclick = async () => { const type = button.dataset.factoryExport; const content = serialize(undefined, type); if (type === 'copy') { await navigator.clipboard.writeText(serialize()); button.innerHTML = `${ic('check')} Tersalin`; setTimeout(() => { button.textContent = 'Copy'; }, 1200); return; } const blob = new Blob([content], { type: type === 'json' ? 'application/json' : 'text/plain' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `ai-content-factory.${type}`; link.click(); URL.revokeObjectURL(link.href); });
