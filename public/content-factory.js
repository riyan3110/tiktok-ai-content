(() => {
  ...
  function storeResults(factoryResult) {
    ...
    const workflows = read('ai-ads-lab-workflow-history-v1'); workflows.unshift(...factoryResult.items.map(item => ({ id: item.id, name: item.title, status: 'Completed', source: 'AI Content Factory', createdAt: item.createdAt, payload: item }))); localStorage.setItem('ai-ads-lab-workflow-history-v1', JSON.stringify(workflows.slice(0, 200)));
