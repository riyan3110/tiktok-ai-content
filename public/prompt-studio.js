(function (root, factory) {
  ...
  const Icons = window.Icons || {}; const ic = (name, cls) => Icons.svg ? Icons.svg(name, cls) : '';
    ...
    container.innerHTML = `<div class="prompt-heading"><div><span class="eyebrow">PROMPT STUDIO</span><h2>Prompt Studio</h2><p>Susun, simpan, dan kembangkan prompt untuk setiap target AI.</p></div><button data-create-prompt type="button">${ic('plus')} Create Prompt</button></div>
      <div class="prompt-toolbar"><label class="project-search"><span aria-hidden="true">${ic('search')}</span><span class="sr-only">Cari prompt</span><input data-prompt-search type="search" placeholder="Cari judul, tag, kategori, atau target AI…"></label><select data-prompt-category aria-label="Filter kategori"><option value="">Semua kategori</option>${options(categories)}</select><select data-prompt-target aria-label="Filter target AI"><option value="">Semua target AI</option>${options(targets)}</select><label class="favorite-filter"><input data-prompt-favorite type="checkbox"> ${ic('star')} Favorite</label></div>
    ...
    function renderList() {
      ...
      if (!prompts.length) $('[data-prompt-list]').innerHTML = `<div class="prompt-empty"><span aria-hidden="true">${ic('terminal')}</span><h3>Belum ada prompt</h3><p>Buat prompt pertama untuk project ini dan mulai versioning secara lokal.</p><button data-empty-create type="button">Create Prompt</button></div>`;
      else if (!visible.length) $('[data-prompt-list]').innerHTML = `<div class="prompt-empty compact"><span aria-hidden="true">${ic('search')}</span><h3>Prompt tidak ditemukan</h3><p>Ubah pencarian atau filter Anda.</p></div>`;
      else $('[data-prompt-list]').innerHTML = `<div class="prompt-table"><div class="prompt-row prompt-table-head"><span></span><span>Judul</span><span>Jenis Prompt</span><span>Target AI</span><span>Versi</span><span>Status</span><span>Last Edited</span></div>${visible.map(p => `<button class="prompt-row" data-prompt-id="${p.id}" type="button"><span class="favorite-star" data-favorite="${p.id}" aria-label="Favorite">${p.favorite ? ic('star', 'icon-filled') : ic('star')}</span><strong>${safe(p.title)}</strong><span>${safe(p.category)}</span><span>${safe(p.target)}</span><span>v${p.version || 1}</span><span class="status-pill"><i></i>${safe(p.status || 'Draft')}</span><time>${new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium' }).format(new Date(p.updatedAt))}</time></button>`).join('')}</div>`;
