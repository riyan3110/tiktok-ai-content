(() => {
  ...
  function recover() {
    ...
    try {
      ...
      $('#workflow-recovery').innerHTML = `<b>Sesi dipulihkan.</b><span>Draft terakhir dilanjutkan dari penyimpanan lokal.</span><button type="button" data-dismiss-recovery aria-label="Tutup">${ic('x')}</button>`;
      ...
      $('#workflow-recovery').innerHTML = `<b>Draft rusak dipulihkan.</b><span>Data yang tidak valid diisolasi dan workflow baru dibuat.</span><button type="button" data-dismiss-recovery aria-label="Tutup">${ic('x')}</button>`;
      $('#workflow-recovery').classList.remove('hidden');
      $('[data-dismiss-recovery]').onclick = () => $('#workflow-recovery').classList.add('hidden');
    }
  }
  ...
  function render() {
    ...
    $('#workflow-stepper').innerHTML = steps.map((item,index) => `<button type="button" data-workflow-step="${index}" class="${index===state.currentStep?'active':''} ${validate(index).length===0?'complete':''}" aria-current="${index===state.currentStep?'step':'false'}"><span>${validate(index).length===0?ic('check'):index+1}</span><b>${item.label}</b><small>${item.description}</small></button>`).join('');
    $('#workflow-panel-title').textContent=step.label; $('#workflow-state').innerHTML=fields(step); $('#workflow-position').textContent=`Langkah ${state.currentStep+1} dari ${steps.length}`; $('#workflow-previous').disabled=state.currentStep===0; $('#workflow-next').innerHTML=state.currentStep===steps.length-1?'Tinjau ringkasan':`Berikutnya ${ic('arrow-right')}`;
