(() => {
  ...
  function loadWorkflow(item) {
    ...
    if (!confirm('Simpan draft aktif lalu lanjutkan workflow ini?')) return; persist(true); state = JSON.parse(JSON.stringify(n.raw)); state.status = 'Draft'; undoStack=[]; redoStack=[]; persist(true); render();
  }
  function useFactory(item) { const mapped=historyApi.mapContentFactoryToWorkflow(item,makeId); if (!mapped) return alert('Record Content Factory tidak valid.'); if (!confirm('Simpan draft aktif dan gunakan konten ini sebagai workflow baru?')) return; persist(true); state=mapped; undoStack=[]; redoStack=[]; persist(true); render(); }
  function deleteHistory(id, type) { const records=historyRecords(), target=records.map(historyApi.normalizeHistoryItem).find(x=>x.id===id&&x.recordType===type); if (!target || !confirm(`Hapus “${target.name}” dari history?`)) return; if (state.id===id && !confirm('Record ini adalah draft aktif. Hapus juga draft aktif dan mulai draft baru?')) return; writeHistory(records.filter(x=>{const n=historyApi.normalizeHistoryItem(x);return !(n.id===id&&n.recordType===type);})); if(state.id===id){localStorage.removeItem(STORAGE_KEY);state=defaults();persist(true);} renderHistory(); }
