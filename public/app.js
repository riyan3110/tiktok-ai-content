...
function validateSourceUrls() { $('#source-url-error').textContent = ''; if (!$('#use-source-urls').checked) return []; const urls = cleanSourceUrls(); if (!urls.length) { $('#source-url-error').textContent = 'Minimal 1 URL sumber wajib diisi.'; return null; } if (urls.length > 3) { $('#source-url-error').textContent = 'Maksimal 3 URL sumber.'; return null; } for (const url of urls) { try { const parsed = new URL(url); if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error(); } catch { $('#source-url-error').textContent = `URL tidak valid: ${url}`; return null; } } return urls; }
function renderSourcePreview(item) { const meta = item.render_source || {}; const sources = meta.sources || []; const host = $('#source-preview'); host.classList.toggle('hidden', !sources.length); if (!sources.length) return; const status = meta.verificationStatus === 'needs_review' ? 'Perlu ditinjau — sumber tidak cukup atau saling berbeda' : 'Berbasis sumber — tetap periksa sebelum dipublikasikan'; host.innerHTML = `<h3>Dibuat berdasarkan sumber</h3><p>${status} · ${sources.length} sumber</p><ul>${sources.map(src => { let domain = src.finalUrl || src.url; try { domain = new URL(domain).hostname; } catch {} return `<li><a href="${escapeHtml(src.finalUrl || src.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(src.title || domain)}</a><br><small>${escapeHtml(domain)} · diambil ${escapeHtml(new Date(src.fetchedAt).toLocaleString('id-ID'))}</small></li>`; }).join('')}</ul>`; }
let sourceUrls = [''];
document.querySelectorAll('input[name="topic-source"]').forEach((input) => input.onchange = () => { const manual = input.value === 'manual' && input.checked; $('#manual-topic-wrap').classList.toggle('hidden', !manual); if (!manual) $('#use-source-urls').checked = false; renderSourceUrlFields(); });
$('#use-source-urls').onchange = renderSourceUrlFields; $('#add-source-url').onclick = () => { if (sourceUrls.length < 3) sourceUrls.push(''); renderSourceUrlFields(); }; renderSourceUrlFields();
