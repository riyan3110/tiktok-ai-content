(() => {
  ...
  function detail(job) { selectedId = job.id; $('#job-detail').classList.remove('hidden'); $('#job-detail').innerHTML = `<div class="detail-heading"><div><span class="eyebrow">JOB DETAIL</span><h2>${safe(job.id)}</h2></div><button class="icon-button" data-close-detail>${ic('x')}</button></div><div class="detail-progress"><div><b>${safe(job.status)}</b><span>Estimated time: ${terminal.includes(job.status) ? 'Complete' : `${Math.ceil((100-job.progress)/20)*2}s`}</span></div><div class="queue-progress"><i style="width:${job.progress}%"></i></div><strong>${job.progress}%</strong></div><div class="job-detail-grid"><section><h3>Prompt</h3><pre>${safe(job.prompt || 'No prompt supplied')}</pre><h3>Provider & Model</h3><p>${safe(job.provider)} · ${safe(job.model)}</p><h3>Output Preview (Mock)</h3><div class="mock-output">${job.result ? `<span>${ic('sparkles')}</span><b>${safe(job.result.preview)}</b><code>${safe(job.result.asset)}</code>` : '<span class="spinner"></span><b>Output will appear after processing</b>'}</div></section><section><h3>Timeline</h3><ol class="job-timeline">${job.timeline.map(item => `<li><i></i><span><b>${safe(item.status)}</b><small>${time(item.at)}</small></span></li>`).join('')}</ol><h3>Logs</h3><div class="job-logs">${job.logs.map(item => `<p><time>${time(item.at)}</time> ${safe(item.message)}</p>`).join('')}</div></section></div>`; $('[data-close-detail]').onclick = () => $('#job-detail').classList.add('hidden'); }
  ...
  function render() {
    ...
    $('#queue-empty').classList.toggle('hidden', jobs.length > 0); $('.queue-table').classList.toggle('hidden', !jobs.length); $('#queue-pause').classList.toggle('hidden', settings.paused); $('#queue-resume').classList.toggle('hidden', !settings.paused); $('#queue-worker-state').innerHTML = settings.paused ? `${ic('pause')} Worker paused` : `${ic('activity')} Worker active`;
