(() => {
  ...
  const state = { project:'', character:'', product:'', style:'', voice:'', target:'Google Flow', language:'English', type:'UGC', duration:'15 seconds', ratio:'9:16', platform:'TikTok', scene:'Hook, product demonstration, social proof, and a clear call to action.', assetIds: [], assetLabels: [], enabled: defaultEnabled, ...read(KEYS.generator, {}) };
  ...
  function renderAssets() { $('#generator-assets').innerHTML = state.assetLabels?.map(asset => `<span class="selected-asset"><img src="${safe(asset.previewUrl)}" alt=""><b>${safe(asset.name)}</b><button type="button" data-remove-generator-asset="${safe(asset.id)}" aria-label="Remove ${safe(asset.name)}">${ic('x')}</button></span>`).join('') || '<small>No assets attached</small>'; document.querySelectorAll('[data-remove-generator-asset]').forEach(button => button.onclick = () => { state.assetIds = state.assetIds.filter(id => id !== button.dataset.removeGeneratorAsset); state.assetLabels = state.assetLabels.filter(asset => asset.id !== button.dataset.removeGeneratorAsset); persist($('#generated-prompt').value); renderAssets(); }); }
